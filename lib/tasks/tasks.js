...
const compareDateTime = (timezone, operation_hours) => {
	return new Promise((resolve, reject) => {
		...
		let prev_last_closing_period =  previous_operation_hours.periods[previous_operation_hours.periods.length - 1] ? 
		moment(`${date_split_prev[0]}, ${previous_operation_hours.periods[previous_operation_hours.periods.length - 1].close}`, 'DD/MM/YYYY, h:mm:ss A') : null;
