...
const pi = require('../../api/routes/remote-update');
...
const timerReset = async (io) => {
	try {
		...
		if (on_off == 1) {
			...
			console.log('#timerReset_Undefined:', on_off);
			io.emit('PS_operation_error');
		}
	...
	try {
		...
		if (init_host_info) {
			...
			if (current_time === "00:00:00") {
				console.log("Restarting Player: ", current_time);
				pi.initiatePiRestart();
				// restartAndUpdate();
			} else {
...
const compareDateTime = (timezone, operation_hours) => {
	return new Promise((resolve, reject) => {
		...
		let previous_day = moment.tz(timezone).subtract(1, 'days').format("dddd");
		let previous_date_time = moment.tz(timezone).subtract(1, 'days').format("DD/MM/YYYY, h:mm:ss A");

		console.log(previous_day);

		let date_split = current_date_time.split(', ');
		let date_split_prev = previous_date_time.split(', ');

		let current_operation_hours = operation_hours.filter(i => i.day === current_day)[0];
		let previous_operation_hours = operation_hours.filter(i => i.day === previous_day)[0];

		let prev_last_closing_period =  moment(`${date_split_prev[0]}, ${previous_operation_hours.periods[previous_operation_hours.periods.length - 1].close}`, 'DD/MM/YYYY, h:mm:ss A');
		
		if (previous_operation_hours.status && previous_operation_hours.periods[previous_operation_hours.periods.length - 1]) {
			const prev_open_hours = moment(`${date_split_prev[0]}, ${previous_operation_hours.periods[previous_operation_hours.periods.length - 1].open}`, 'DD/MM/YYYY, h:mm:ss A');
			const prev_close_hours = moment(prev_last_closing_period, 'h:mm:ss A') > moment(prev_open_hours, 'h:mm:ss A') ? moment(prev_last_closing_period, 'DD/MM/YYYY, h:mm:ss A') : moment(prev_last_closing_period, 'DD/MM/YYYY, h:mm:ss A').add(1, 'd');
			...
			if (prev_close_hours > now) {
				console.log('IS OPEN FROM YESTERDAY');
				status = 1;
			} else {
				console.log('IS CLOSE FROM YESTERDAY, CHECKING SCHEDULE FOR TODAY');
